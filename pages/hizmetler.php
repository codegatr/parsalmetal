    ...
    <div class="hizmet-detay-wrap" style="display:grid;grid-template-columns:1fr 300px;gap:48px;align-items:start">
      ...
      <div class="service-sidebar">
        <div style="background:linear-gradient(145deg,#c0392b 0%,#96281b 100%);border-radius:16px;padding:28px;color:#fff">
          ...
          <p style="font-size:13px;color:rgba(255,255,255,.8);line-height:1.6;margin:0 0 20px">Uzman ekibimiz en kısa sürede sizinle iletişime geçecektir.</p>
          <a href="/?page=teklif&service=<?= urlencode($service['name']) ?>" style="display:block;background:#fff;color:#c0392b;text-align:center;padding:12px;border-radius:10px;font-weight:700;font-size:13px;text-decoration:none;margin-bottom:10px">Hemen Teklif Al →</a>
          <?php if ($phone): ?>
        ...
        <div style="background:#f8f9fa;border-radius:16px;padding:24px">
          ...
          <a href="/?page=hizmetler&slug=<?= htmlspecialchars($s['slug']) ?>" style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid #eee;text-decoration:none;color:#444;font-size:13px;font-weight:500" onmouseover="this.style.color='#c0392b'" onmouseout="this.style.color='#444'">
      ...
      <div class="service-card">
        ...
        <div class="service-body">
          ...
          <a href="/?page=hizmetler&slug=<?= htmlspecialchars($s['slug']) ?>" class="service-link" style="margin-top:16px">
            Detaylar <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
          </a>
        </div>
      </div>
      <?php endforeach; ?>
